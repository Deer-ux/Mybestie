...
const CATEGORIES: { id: MessageCategory; label: string; emoji: string; color: string; placeholder: string }[] = [
  { id: 'compliment',   label: 'Compliment',   emoji: '❤️',  color: '#FF6B6B', placeholder: 'Tell them something you genuinely admire about them...' },
  { id: 'question',     label: 'Question',     emoji: '🤔',  color: '#38BDF8', placeholder: "Ask something you've always wanted to know..." },
  { id: 'advice',       label: 'Advice',       emoji: '💡',  color: '#F59E0B', placeholder: 'Share advice you think they need to hear...' },
  { id: 'confession',   label: 'Confession',   emoji: '🤫',  color: '#7B2CFF', placeholder: "Say something you've never told them out loud..." },
  { id: 'encouragement',label: 'Encouragement',emoji: '✨',  color: '#00FF88', placeholder: 'Give them the push they need right now...' },
  { id: 'feedback',     label: 'Feedback',     emoji: '📝',  color: '#A29BFE', placeholder: 'Give them real, constructive feedback...' },
  { id: 'other',        label: 'Just Say It',  emoji: '💬',  color: '#9CA3AF', placeholder: 'Just say it — anything on your mind...' },
];

const EMOJIS = ['😊', '❤️', '🔥', '✨', '💯', '😂', '🤔', '👀', '💙', '🙌', '😍', '💪', '🥺', '😭', '🌟', '👏'];

export default function PublicMessageScreen() {
  ...
  useEffect(() => { trackEvent('public_link_visited'); }, []);
  ...
  const slug         = username ?? '';
  const displayName  = slug || 'Someone';
  ...
  async function handleCreateMyLink() {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    trackEvent('viral_registration_from_public');
    router.replace('/');
  }
